 ...
 * - überschreibt die Datei nur, wenn debugMode === true
 */
function generateInterface(obj: Record<string, any>, interfaceName: string, outFile: string, debugMode: boolean) {
  ...
  if (exists && !debugMode) {
    console.log(`📌 InterfaceSniper: Datei existiert bereits und debugMode=false → nicht überschrieben: ${outFile}`);
    return;
  }
  ...
  // Schreiben (überschreibt wenn debugMode true oder Datei nicht existiert)
  fs.writeFileSync(outFile, interfaceDef, { encoding: "utf8" });

  console.log(`${exists ? (debugMode ? "🔁 Interface (überschrieben)" : "📌 Interface (nicht überschrieben)") : "📄 Interface (neu)"} ${interfaceName} geschrieben nach ${outFile}`);
}
...
/**
 * Method-Decorator: erzeugt ein Interface für den Rückgabewert der Methode.
